...
class PillDataset(Dataset):
    def __init__(self, data, image_folder, transform):
        ...
        self.label_encoder = {name: idx for idx, name in
                              enumerate(data['품목명'].unique())}  # 문자열 형태의 클래스 레이블(예: 품목명)을 정수 형태의 인덱스로 변환합니다.
        self.label_decoder = {idx: name for name, idx in self.label_encoder.items()}  # 역인덱싱을 위한 label_decoder
    ...
    def __getitem__(self, idx):
        ...
        pill_name = row['품목명']
        image_name = f"{pill_name}_{row.name}.jpg"
...
# 학습이 끝난 후, label_decoder를 pickle로 저장
with open('label_decoder.pkl', 'wb') as f:
    pickle.dump(train_dataset.label_decoder, f)

print("Training finished, best model saved. Label decoder saved as 'label_decoder.pkl'.")
    # # 검증
    # model.eval()
    # val_loss = 0
    # correct = 0
